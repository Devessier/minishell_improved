...
/*   Updated: 2019/02/22 14:23:03 by bdevessi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	variable_name_check(int c)
{
...
int			sh_builtin_setenv(t_string *args, size_t len, t_env *env)
{
	...
	if (len > 2 || !(ft_isalpha(*args[0].buff) || *args[0].buff == '_')
		|| !ft_stritercheck(args[0].buff, variable_name_check))
	{
		if (!(ft_isalpha(*args[0].buff) || *args[0].buff == '_'))
			ft_putstr("minishell: setenv: variable name must begin"
				" with a letter or an underscore\n");
		else
			ft_putstr(len > 2 ? "minishell: setenv: too many arguments"
				: "minishell: setenv: variable name must only"
